...
//middlewares ทำการคิวรี ข้อมูลจาก Blog
...
router.post('/:blogId/comments', isLoggedIn, async function (req, res, next) {//+ isLoggedIn
...
// Delete comment
router.delete('/comments/:commentId', isLoggedIn, CommentOwner, async function (req, res, next) {//+ isLoggedIn
    try {
        const [rows1, fields1] = await pool.query(
